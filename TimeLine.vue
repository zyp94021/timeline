...
function Line(ctx, start, end, { y = 100, lineWidth = 2, lineHeight = 10, color = '#d9d9d9' }) {
  this.ctx = ctx
  this.start = start
  this.y = y
  this.end = end
...
export default {
  ...
  methods: {
    ...
    drawLine(start, end) {
      ...
      this.ctx.fillRect(start, this.height - this.lineHeight, end - start, this.lineHeight)
    ...
    drawText(start, end, text) {
      ...
      for (let i = 0; i < lineArr.length; i++) {
        this.ctx.fillText(lineArr[i], (start + end) / 2, this.height - (lineArr.length - i) * 16, end - start)
      }
    }
  }
}
